...
import { ChevronLeft, Trash2, Plus, ChevronRight, MapPin, Cloud, Clock, Image as ImageIcon } from 'lucide-react';
...
const Editor = ({ entry, onClose, onSave, onDelete }) => {
  ...
  const handleLocation = () => {
    if (!navigator.onLine) {
      ...
      manual && setLocation(manual);
      return;
    }
    if (!navigator.geolocation) {
      const manual = window.prompt("Geolocation not supported. Enter manually:");
      manual && setLocation(manual);
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          weatherData.current_weather?.temperature && setWeather(`${weatherData.current_weather.temperature}°C`);
          ...
          setLocation(city ? (locData.countryName ? `${city}, ${locData.countryName}` : city) : "Unknown Location");
        } catch {
          const manual = window.prompt("Could not fetch location. Enter manually:");
          manual && setLocation(manual);
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col animate-slideUp overflow-hidden" style={{ height: '100dvh' }}>
      <div className="px-4 py-3 flex justify-between items-center bg-white/80 backdrop-blur-md absolute top-0 left-0 right-0 z-20 border-b border-gray-100/50">
        <button onClick={onClose} className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors"><ChevronLeft size={24} /></button>
        <div className="flex items-center gap-2">
          {entry?.id &&
            <button onClick={handleDelete} className="p-2 text-red-500 hover:bg-red-50 rounded-full transition-colors"><Trash2 size={20} /></button>
          }
          <button onClick={handleSave} className="px-4 py-1.5 bg-blue-500 text-white font-semibold rounded-full shadow-md shadow-blue-500/20 active:scale-95 transition-all text-sm">Done</button>
        </div>
      </div>
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto no-scrollbar pt-16">
        {images.length > 0 && (<div className="w-full h-72 relative group bg-gray-100 mb-4">
          ...
          {images.length > 1 &&
            <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
              {images.map((_, i) => <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === imgIndex ? 'bg-white w-3' : 'bg-white/50'}`} />)}
            </div>
          }
        </div>)
        }
        <div className="px-6 pb-12">
          <div className="mb-6">
            ...
            <div className="flex items-center gap-2 text-gray-400 text-sm mt-1 font-medium">
              <Clock size={14} />
              {entryDate.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}<span>•</span>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="relative">
              ...
              {isMoodOpen && (<MoodPopup currentMood={mood} onChange={setMood} onClose={() => setIsMoodOpen(false)} />)}
            </div>
            ...
            {weather &&
              <div className="flex items-center bg-orange-50 text-orange-600 rounded-full px-2 py-0.5 border border-orange-100 text-xs">
                <Cloud size={12} className="mr-1" />{weather}
              </div>
            }
            <TagInput tags={tags} onAdd={t => setTags([...tags, t])} onRemove={t => setTags(tags.filter(tag => tag !== t))} />
          </div>
          <textarea ref={textareaRef} value={content} onChange={e => setContent(e.target.value)} placeholder="Start writing..." className="w-full min-h-[300px] resize-none text-lg text-gray-800 placeholder-gray-300 border-none p-0 focus:ring-0 outline-none leading-7 font-serif bg-transparent pb-20 overflow-hidden" />
